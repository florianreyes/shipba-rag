...
export const createUser = async (input: NewUserParams) => {
  ...
  try {
    var { name, mail, content, x_handle, telegram_handle, instagram_handle } = insertUserSchema.parse(input);
    ...
    const [user] = await db
      .insert(users)
      .values({ name, mail, content, x_handle, telegram_handle, instagram_handle })
      .returning();
